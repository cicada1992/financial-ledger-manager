...
import { IAnnually, IUpdateAnnuallyBody } from '@/app/api/AnnuallyAPI/types';
import ButtonWithModal from '@/app/components/ButtonWithModal';
import { useUserStore } from '@/app/store/userStore';
import { YYYYMMDD } from '@/types';

import AnnuallyRowForm from './shared/AnnuallyForm';
...
interface IProps {
  ...
  type: IAnnually['type'];
  className?: string;
  onEdit(row: IAnnually): void;
}

const AnnuallyRowEditor: React.FC<IProps> = ({ row, type, className, onEdit }) => {
  ...
  function handleEditClick() {
    ...
    const body: IUpdateAnnuallyBody = {
      id: row.id,
      name,
...
export default AnnuallyRowEditor;
